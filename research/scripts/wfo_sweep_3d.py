...
ARMS = ["sweep_v1", "sweep_v2"]  # A=baseline, B=+insider/institutional
START, END = "2024-07-01", "2026-07-02"
OUT_DIR = RIG / ".yats_data" / "wfo_sweeps" / "sweep_3d_insider_ab"
...
def mark(msg: str) -> None:
    print(f"[3D] {datetime.now(timezone.utc).strftime('%H:%M:%S')} {msg}", flush=True)
...
def make_spec(cfg_idx: int, params: dict, feature_set: str) -> ExperimentSpec:
    return ExperimentSpec(
        experiment_name=f"wfo3d_{feature_set}_cfg{cfg_idx}",
        ...
        notes="Stage 3d insider/institutional A/B WFO sweep — matched purge 105 both arms",
    )
...
def main() -> int:
    ...
    mark(f"A/B SWEEP start: arms={ARMS}, {len(GRID)} configs each, purge=1+105 matched")
    ...
    summary = {
        "sweep": "3d_insider_ab_anchored_wfo",
        "grid_size": len(GRID),
        "arms": ARMS,
        "symbols": SYMBOLS,
        "span": [START, END],
