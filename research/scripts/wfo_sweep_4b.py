...
ARMS = ["sweep_v1", "sweep_v1r", "sweep_v3"]  # A=baseline, B=+regime_v2 (PRIMARY), C=full stack (fold-4 interaction)
START, END = "2024-07-01", "2026-07-02"
OUT_DIR = RIG / ".yats_data" / "wfo_sweeps" / "sweep_4b_regime_abc"
...
def mark(msg: str) -> None:
    print(f"[4B] {datetime.now(timezone.utc).strftime('%H:%M:%S')} {msg}", flush=True)


def fetch_env_rows(feature_set):
    """Fetch env rows for the given feature set (with feature_set filter)."""
    ...
    mark(f"DATA {feature_set}: rows={len(rows)} obs_cols={len(obs_cols)} regime={len(regime_cols)} dropped={n_dropped}")
    return rows, obs_cols, regime_cols


def make_spec(cfg_idx: int, params: dict, feature_set: str) -> ExperimentSpec:
    return ExperimentSpec(
        experiment_name=f"wfo4b_{feature_set}_cfg{cfg_idx}",
        ...
        seed=42,
        wfo_config=WFOConfig(train_window=250, purge_buffer=105),
        notes="Stage 4b regime_v2 3-arm WFO sweep — matched purge 105 all arms",
    )
...
def main() -> int:
    ...
    # MATCHED geometry across arms: explicit purge_buffer=105 (sweep_v2's
    # feature memory) for BOTH — the baseline arm deviates from auto-sizing
    # (64) so the OOS windows are identical and the comparison is clean.
    wfo_cfg = WFOConfig(train_window=250, purge_buffer=105)
    mark(f"4B SWEEP start: arms={ARMS}, {len(GRID)} configs each, purge=1+105 matched")

    per_config = []
    for feature_set in ARMS:
      data, obs_cols, regime_cols = fetch_env_rows(feature_set)
      for i, params in enumerate(GRID):
        cfg_t0 = time.time()
        spec = make_spec(i, params, feature_set)
        ckpt_dir = OUT_DIR / f"{feature_set}_cfg{i}"
        ...
        try:
            ...
            mark(f"CONFIG {feature_set}/{i} FAILED: {exc}")
            per_config.append({"arm": feature_set, "config_index": i, "params": params, "failed": str(exc)})
            continue
        ...
        per_config.append({
            "arm": feature_set, "config_index": i, "params": params,
        ...
        mark(f"CONFIG {feature_set}/{i} done: OOS sharpe={sharpe:.3f} folds={['%.2f' % (s or 0) for s in result.per_fold_oos_sharpe]} "
    ...
    summary = {
        "sweep": "4b_regime_abc_anchored_wfo",
        "grid_size": len(GRID),
        "arms": ARMS,
        "symbols": SYMBOLS,
        "span": [START, END],
        "wfo": {"mode": "anchored", "n_periods": 4, "purge": "1+105 matched"},
        "configs": per_config,
